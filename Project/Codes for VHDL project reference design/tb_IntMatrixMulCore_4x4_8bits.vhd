...
-- Output matrix: C - Size: 4 x 4 - 18 bits
--
...
component IntMatMulCore
	port(
		...
		ReadData: 		out std_logic_vector (17 downto 0);
...
signal tb_ReadData : std_logic_vector(17 downto 0);
...
begin
	...
	begin
		...
		while not ENDFILE(FI)  loop
			...
			if v_ReadDatam = tb_ReadData(17)&tb_ReadData(17)&tb_ReadData then
				v_OK := True;
			else
				v_OK := False;
			end if;
			HWRITE(L, v_ReadDatam, Left, 20);
			HWRITE(L, tb_ReadData(17)&tb_ReadData(17)&tb_ReadData, Left, 20);
